...
bool HomeAssistantComponent::can_update_from_api() {
  if (ignore_update_until_ == 0) {
    ESP_LOGD(TAG, "can_update_from_api: true %@ %d", get_entity_id().c_str(),
             ignore_update_until_);
    return true;
  } else {
    ESP_LOGD(TAG, "can_update_from_api: %@ %d", get_entity_id().c_str(),
...
void HomeAssistantComponent::ignore_api_updates_with_seconds(int seconds) {
  ESP_LOGD(TAG, "ignore_api_updates_with_seconds: %@ %d",
