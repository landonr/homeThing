...
class HomeAssistantNumber
  ...
  void publish_api_state(float state) {
    ...
    ESP_LOGI(TAG, "publish_api_state: '%s' %f", this->get_name().c_str(),
             state);
  ...
  void state_changed(std::string state) {
    ESP_LOGI(TAG, "'%s': state_changed %s", get_name().c_str(), state.c_str());
  ...
  void control(float value) {
    ESP_LOGI(TAG, "'%s': control %f", get_name().c_str(), value);
    publish_state(value);
