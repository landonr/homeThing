...
class BasePlayerComponent : public CustomAPIDevice, public Component {
 ...
 public:
  ...
  void superSetup() {
    ...
    subscribe_homeassistant_state(&BasePlayerComponent::playerState_changed,
                                  entityId.c_str());
  }

  void playSource(MenuTitleSource source) {
    ESP_LOGI("Player", "Player play source %s %s %s %d", entityId.c_str(),
             source.entityId.c_str(), source.sourceTypeString().c_str(),
             source.sourceType);
 ...
 private:
  void selectSource(MenuTitleSource source) {
    ESP_LOGI("player", "%s select source %s", entityId.c_str(),
             source.friendlyName.c_str());
  ...
  void playMedia(MenuTitleSource source) {
    ESP_LOGI("player", "%s play media %s %s", entityId.c_str(),
             source.friendlyName.c_str(), source.entityId.c_str());
  ...
  void playerState_changed(std::string state) {
    ESP_LOGI("Player", "%s state changed to %s", entityId.c_str(),
             state.c_str());
    if (state.length() == 0) {
...
class SonosSpeakerComponent : public BasePlayerComponent {
 public:
  ...
  void setup() {
    ...
    subscribe_homeassistant_state(
        &SonosSpeakerComponent::speaker_volume_changed, entityId,
        "volume_level");
    subscribe_homeassistant_state(&SonosSpeakerComponent::speaker_muted_changed,
                                  entityId, "is_volume_muted");
    subscribe_homeassistant_state(&SonosSpeakerComponent::shuffle_changed,
                                  entityId, "shuffle");
    subscribe_homeassistant_state(&SonosSpeakerComponent::group_members_changed,
                                  entityId, "group_members");
    subscribe_homeassistant_state(
        &SonosSpeakerComponent::player_media_title_changed, entityId,
        "media_title");
    subscribe_homeassistant_state(
        &SonosSpeakerComponent::player_media_artist_changed, entityId,
        "media_artist");
    subscribe_homeassistant_state(&SonosSpeakerComponent::playlist_changed,
                                  entityId, "media_playlist");
    subscribe_homeassistant_state(&SonosSpeakerComponent::media_album_changed,
                                  entityId, "media_album_name");
    subscribe_homeassistant_state(
        &SonosSpeakerComponent::media_duration_changed, entityId,
        "media_duration");
    subscribe_homeassistant_state(
        &SonosSpeakerComponent::media_position_changed, entityId,
        "media_position");
    subscribe_homeassistant_state(&SonosSpeakerComponent::media_source_changed,
  ...
  void joinGroup(std::string newSpeakerName) {
    ESP_LOGI("speaker", "%s group speaker to %s", entityId.c_str(),
 ...
 private:
  void player_media_title_changed(std::string state) {
    ESP_LOGI("Player", "%s Player media title changed to %s", entityId.c_str(),
             state.c_str());
  ...
  void speaker_volume_changed(std::string state) {
    ESP_LOGI("speaker", "%s Sonos Speaker volume changed to %s",
  ...
  void media_source_changed(std::string state) {
    ESP_LOGI("speaker", "%s Speaker source changed to %s", entityId.c_str(),
             state.c_str());
...
class TVPlayerComponent : public BasePlayerComponent {
 public:
  ...
  void setup() {
    ...
    superSetup();
    subscribe_homeassistant_state(&TVPlayerComponent::player_source_changed,
                                  entityId, "source");
    subscribe_homeassistant_state(
        &TVPlayerComponent::player_source_list_changed, entityId,
        "source_list");
  }

  void player_source_changed(std::string state) {
    ESP_LOGI("PlayerTV", "%s Player source changed to %s", friendlyName.c_str(),
             state.c_str());
  ...
  void player_source_list_changed(std::string state) {
    ESP_LOGI("PlayerTV", "%s Player source list changed to %s",
             friendlyName.c_str(), state.c_str());
...
class SonosSpeakerGroupComponent : public CustomAPIDevice,
                                   public Component,
 ...
 public:
  ...
  void findActivePlayer(bool background = false) {
    ...
    if (newActivePlayer != NULL) {
      ESP_LOGI("FIND_ACTIVE_PLAYER", "setting active player %s",
               newActivePlayer->entityId.c_str());
  ...
  void setActivePlayer(BasePlayerComponent* newActivePlayer) {
    ESP_LOGI("ACTIVE_PLAYER", "New active player %s",
             newActivePlayer->entityId.c_str());
  ...
  void setup(std::vector<TVSetup> newTVSetups,
    ...
    subscribe_homeassistant_state(
        &SonosSpeakerGroupComponent::playlists_changed,
        "sensor.playlists_sensor", "playlists");
    subscribe_homeassistant_state(
        &SonosSpeakerGroupComponent::sonos_favorites_changed,
  ...
  void increaseSpeakerVolume() {
    if (activePlayer->playerType != TVRemotePlayerType) {
      SonosSpeakerComponent* activeSpeaker =
          static_cast<SonosSpeakerComponent*>(activePlayer);
      if (activeSpeaker != NULL) {
        activeSpeaker->increaseVolume();
      }
    } else {
      TVPlayerComponent* activeTV =
          static_cast<TVPlayerComponent*>(activePlayer);
      if (activeTV != NULL) {
  ...
  std::vector<MenuTitlePlayer*> groupTitleSwitches() {
    ...
    out.push_back(new MenuTitlePlayer(
        "Group " + newSpeakerGroupParent->friendlyName,
        newSpeakerGroupParent->entityId, NoMenuTitleLeftIcon,
    ...
    for (auto& speaker : speakers) {
      if (newSpeakerGroupParent->entityId == speaker->entityId) {
        ...
        if (std::find(speaker->groupMembers.begin(),
                      speaker->groupMembers.end(),
                      newSpeakerGroupParent->entityId) !=
            speaker->groupMembers.end()) {
          out.push_back(
              new MenuTitlePlayer(speaker->friendlyName, speaker->entityId,
                                  OnMenuTitleLeftIcon, NoMenuTitleRightIcon,
                                  speaker->mediaSource, speaker->playerState));
        } else {
          out.push_back(
              new MenuTitlePlayer(speaker->friendlyName, speaker->entityId,
                                  OffMenuTitleLeftIcon, NoMenuTitleRightIcon,
                                  speaker->mediaSource, speaker->playerState));
        }
      }
    }
  ...
  std::vector<MenuTitlePlayer*> groupTitleString() {
    ...
    for (auto& speaker : speakers) {
      if (std::find(groupedMembers.begin(), groupedMembers.end(),
                    speaker->entityId) != groupedMembers.end()) {
        // skip grouped members that were already found
        continue;
      }
      if (speaker->groupMembers.size() > 1) {
        if (speaker->groupMembers[0] != speaker->entityId) {
          ESP_LOGD("group title string", "%s not parent %s",
                   speaker->groupMembers[0].c_str(), speaker->entityId.c_str());
  ...
  std::vector<MenuTitlePlayer*> mediaPlayersTitleString() {
    ...
    for (auto& speaker : speakers) {
      if (speaker->mediaSource != TVRemotePlayerMediaSource) {
        out.push_back(new MenuTitlePlayer(
            speaker->friendlyName, speaker->entityId, NoMenuTitleLeftIcon,
            NoMenuTitleRightIcon, speaker->mediaSource, speaker->playerState));
      }
    }
    for (auto& tv : tvs) {
      out.push_back(new MenuTitlePlayer(
          tv->friendlyName, tv->entityId, NoMenuTitleLeftIcon,
          NoMenuTitleRightIcon, tv->mediaSource, tv->playerState));
      if (tv->speaker != NULL &&
          tv->speaker->mediaSource == TVRemotePlayerMediaSource) {
        out.push_back(new MenuTitlePlayer(
            tv->speaker->friendlyName, tv->speaker->entityId,
  ...
  MenuTitlePlayer headerMediaPlayerTitle() {
    ...
    return MenuTitlePlayer(friendlyName, activePlayer->entityId,
                           NoMenuTitleLeftIcon, NoMenuTitleRightIcon,
                           activePlayer->mediaSource,
  ...
  void selectGroup(MenuTitlePlayer selectedMenuTitle) {
    if (newSpeakerGroupParent == NULL) {
      MenuTitlePlayer* selectedMenuTitleCopy = new MenuTitlePlayer(
          selectedMenuTitle.friendlyName, selectedMenuTitle.entityId,
      ...
      case SpeakerRemotePlayerType: {
        ...
        if (activeSpeaker != NULL) {
          if (activeSpeaker->tv != NULL &&
              activeSpeaker->tv->friendlyName.size() > 0) {
            auto tvSource = std::make_shared<MenuTitleSource>(
                activeSpeaker->tv->friendlyName, "", NoMenuTitleRightIcon,
                SourceRemotePlayerSourceType);
            out.push_back(tvSource);
          }
          out.insert(out.end(), spotifyPlaylists.begin(),
                     spotifyPlaylists.end());
  ...
  virtual void stateUpdated(RemotePlayerState state) {
    ESP_LOGD("SYNC", "state update callback %d %d", activePlayer == NULL,
    ...
    ESP_LOGD("SYNC",
             ...
             state, activePlayer == NULL, id(sync_active_player) == true);
    switch (state) {
      case NoRemotePlayerState:
      case PausedRemotePlayerState:
      case UnavailableRemotePlayerState:
      ...
      case PlayingRemotePlayerState:
        if (activePlayer == NULL) {
          ...
          ESP_LOGD("SYNC", "Trying to sync active player - 3 %d",
                   activePlayer->playerState);
          syncActivePlayer(state);
        }
        return;
