...
class SensorComponent : public CustomAPIDevice, public Component {
 public:
  explicit SensorComponent(FriendlyNameEntity newFriendlyEntity)
      : entityId(newFriendlyEntity.entityId),
        friendlyName(newFriendlyEntity.friendlyName) {
    subscribe_homeassistant_state(&SensorComponent::state_changed,
                                  newFriendlyEntity.entityId.c_str());
  }
  std::string entityId;
 ...
 private:
  void state_changed(std::string newState) {
    ESP_LOGD("Sensor", "%s state changed to %s", friendlyName.c_str(),
             newState.c_str());
    state = newState;
  }
};
  ...
  std::vector<std::shared_ptr<MenuTitleBase>> sensorTitles() {
    ...
    for (auto &sensor : sensors) {
      if (sensor->friendlyName != "") {
        ...
        out.push_back(std::make_shared<MenuTitleBase>(
            sensor->state, sensor->entityId, NoMenuTitleRightIcon));
      }
    }
    return out;
  }
